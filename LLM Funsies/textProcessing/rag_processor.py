...
class RAGProcessor:
    ...
    embedding_model: str
    index: Optional[faiss.Index] = field(default=None)
      ...
      #  print(directory)
       # print(glob.glob(directory))
        for file in glob.iglob(directory,recursive=True):
           # print(file)
            current_document = ChunkedDocument(file,200) #The number is how many tokens in a chunk; the longer this is, the more text we get when we search for something.
...
# /**/*.txt 
# new_rag_processor = RAGProcessor("chunked_document_demo.json", "all-MiniLM-L6-v2")